...
@step(
    name="fetch_quickbooks",
    inputs={"path": str, "sheet_name": object},
    outputs={"gl": pd.DataFrame},
    effects=("read",),
    description=(
        "Read a QuickBooks GL export (Excel/CSV); validate required columns "
        "and strip whitespace on string columns."
    ),
)
def fetch_quickbooks(path: str, sheet_name: int | str = 0) -> dict[str, pd.DataFrame]:
    return {"gl": QuickBooksConnector(path, sheet_name=sheet_name).fetch()}
