...
@step(
    ...
    inputs={"state": dict, "state_version": str},
    outputs={"chunks": list},
    effects=(),
    description="Generate self-contained RAG chunks (scope + per-project + guardrails) from an operating state dict.",
)
def chunk_generation(state: dict, state_version: str = "operating_state_v1") -> dict[str, list]:
    return {"chunks": ChunkGenerationStep(state_version=state_version).run(state)}
