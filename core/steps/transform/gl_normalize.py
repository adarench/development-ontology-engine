...
@step(
    name="gl_normalize",
    inputs={"gl_clean": pd.DataFrame},
    outputs={"gl_normalized": pd.DataFrame},
    effects=(),
    description="Normalize cleaned GL into canonical schema: entity_role, project_id, phase_id, cost_bucket.",
)
def gl_normalize(gl_clean: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {"gl_normalized": GLNormalizeStep().run(gl_clean)}
