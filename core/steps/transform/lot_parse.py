...
@step(
    name="lot_parse",
    inputs={"tasks": pd.DataFrame},
    outputs={"parsed_lots": pd.DataFrame},
    effects=(),
    description="Parse ClickUp task names into project_code / lot_number / stage and assign lot_key.",
)
def lot_parse(tasks: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {"parsed_lots": LotParseStep().run(tasks)}
