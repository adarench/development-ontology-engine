...
@step(
    ...
    inputs={"state": dict, "question": str},
    outputs={"answer": Answer},
    effects=(),
    description="Answer a question against an operating state dict using registered handlers; falls back to keyword search.",
)
def query_execution(state: dict, question: str) -> dict[str, Answer]:
    return {"answer": QueryExecutionStep().run({"state": state, "question": question})}
