...
@step(
    name="lot_state",
    inputs={"parsed_lots": pd.DataFrame},
    outputs={"lot_state": pd.DataFrame},
    effects=(),
    description="Aggregate parsed ClickUp lots into one row per lot with stage progression and completion %.",
)
def lot_state(parsed_lots: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {"lot_state": LotStateStep().run(parsed_lots)}
