...
@step(
    name="phase_cluster",
    inputs={"lot_state": pd.DataFrame, "gap_threshold": int},
    outputs={"clustered_lots": pd.DataFrame},
    effects=("probabilistic",),
    description="Estimate phase IDs by gap-based lot_number clustering. Heuristic; confidence 0.5.",
)
def phase_cluster(lot_state: pd.DataFrame, gap_threshold: int = 10) -> dict[str, pd.DataFrame]:
    return {"clustered_lots": PhaseClusterStep(gap_threshold=gap_threshold).run(lot_state)}
