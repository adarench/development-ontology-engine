---
chunk_id: project_ironton
chunk_type: project
title: Project: Ironton
project: Ironton
source_files:
...
Ironton is an active BCPD project with inventory rows but **no GL coverage**. There are 0 active 2025Status lots; no Vertical Financials rows, no DataRails 38-col rows after the canonical filter, and no Collateral Report row. This is a structural gap, not a v2.1 defect — the cost is **unknown**, not zero. 12 active 2025Status lots; no GL coverage. Cost unknown.
...
- How many active lots does Ironton have in inventory?
- What is the v2.1 status of Ironton (active / no GL / decoder-derived / etc.)?

## Questions to refuse or caveat

- What is Ironton's actual cost? — REFUSE: no GL data; cost is unknown, not zero.
- What is the cost-per-lot for Ironton? — REFUSE: cannot compute without GL.
- Provide org-wide cost including Ironton? — REFUSE: org-wide v2 is blocked.
